...
class SecureSessionStorage:
    ...
    def __init__(self, password: Optional[str] = None, use_encryption: bool = True):
        ...
        if self.use_encryption:
            if not password:
                ...
                logger.warning("Using default password for session encryption. "
            ...
            logger.warning("Cryptography library not available. Falling back to plain JSON. "
    ...
    def _generate_default_password(self) -> str:
        ...
        import platform
        system_info = f"{platform.node()}{platform.system()}{os.getenv('USER', 'default')}"
    ...
    def save_session(self, session_data: Dict[str, Any], session_file: str) -> None:
        ...
        try:
            ...
            logger.error("Failed to save session", session_file=session_file, exc_info=e)
            raise
        ...
        try:
            ...
            if self.use_encryption and self.cipher:
                try:
                    ...
                    logger.debug("Session loaded with decryption", session_file=session_file)
                except Exception:
                    # If decryption fails, try as plain JSON
                    json_data = file_data.decode('utf-8')
                    logger.debug("Session loaded as plain JSON (decryption failed)", 
            ...
            logger.error("Session file corrupted (invalid JSON)", 
                        session_file=session_file, exc_info=e)
            raise ValueError(f"Session file corrupted: {session_file}")
        except Exception as e:
            logger.error("Failed to load session", session_file=session_file, exc_info=e)
            raise
    ...
    def migrate_pickle_session(self, pickle_file: str, json_file: str) -> bool:
        ...
        try:
            ...
            logger.warning("Loading pickle file for migration - this is the last time!", 
            ...
            logger.info("Successfully migrated pickle session to secure format",
                       from_file=pickle_file, to_file=json_file)
            
            return True
            
        except Exception as e:
            logger.error("Failed to migrate pickle session", 
                        pickle_file=pickle_file, exc_info=e)
            return False
...
class LegacyPickleSession:
    ...
    def load_with_warning(session_file: str) -> Dict[str, Any]:
        ...
        logger.critical("SECURITY WARNING: Loading pickle session file! "
...
def get_secure_storage(password: Optional[str] = None, 
                      use_encryption: bool = True) -> SecureSessionStorage:
