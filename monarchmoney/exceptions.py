...
class InvalidMFAError(AuthenticationError):
    ...
    def __init__(self, message: str = "Invalid multi-factor authentication code",
                 details: Optional[Dict[str, Any]] = None):
        super().__init__(message, details)
...
def handle_graphql_errors(errors: list, query: Optional[str] = None) -> None:
    ...
    for error in errors:
        ...
        if any(term in error_msg for term in ['unauthorized', 'unauthenticated', 'invalid token']):
