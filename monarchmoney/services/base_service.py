...
class BaseService:
    ...
    async def _execute_query(self, operation: str, query: Any) -> Dict[str, Any]:
        ...
        try:
            result = await self.client.gql_call(operation=operation, graphql_query=query)
            self.logger.debug("GraphQL operation completed successfully", operation=operation)
            return result
