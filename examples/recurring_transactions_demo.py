...
class RecurringTransactionsDemo:
    ...
    async def demo_get_recurring_streams(self):
        ...
        try:
            ...
            for i, stream in enumerate(streams.get('recurringStreams', [])[:3]):
                ...
                print(f"  Merchant: {stream.get('merchant', {}).get('name', 'Unknown')}")
            ...
            print(f"Found {len(filtered_streams.get('recurringStreams', []))} approved non-liability streams")
    ...
    async def demo_get_aggregated_recurring_items(self):
        ...
        try:
            ...
            for group in items_by_status.get('aggregatedRecurringItems', {}).get('groups', []):
                print(f"  {group.get('groupKey', 'Unknown')}: {group.get('totalCount', 0)} items")
            ...
            for group in items_by_date.get('aggregatedRecurringItems', {}).get('groups', [])[:5]:
                date = group.get('groupKey', 'Unknown')
                count = group.get('totalCount', 0)
                total_amount = sum(item.get('amount', 0) for item in group.get('items', []))
    ...
    async def demo_review_recurring_stream(self):
        ...
        try:
            ...
            pending_streams = [
                s for s in streams.get('recurringStreams', [])
    ...
    async def demo_mark_stream_as_not_recurring(self):
        ...
        try:
            ...
            print(f"Would mark stream as not recurring: {merchant_name} (ID: {stream_id})")
            print(f"Current status: {stream.get('reviewStatus')}")
    ...
    async def demo_get_recurring_merchant_search_status(self):
        ...
        try:
            ...
            print(f"  Status: {status.get('recurringMerchantSearch', {}).get('status', 'Unknown')}")
            print(f"  Last updated: {status.get('recurringMerchantSearch', {}).get('lastUpdated', 'Unknown')}")
    ...
    async def run_demo(self):
        ...
        try:
            ...
            print("  1. get_recurring_streams() - Get all recurring streams with filtering")
            print("  2. get_aggregated_recurring_items() - Get items aggregated by date/status")
            ...
            print("  5. get_recurring_merchant_search_status() - Check search operation status")
            print("  6. get_all_recurring_transaction_items() - Get all items with forecasts")
