...
class TestLogin:
    ...
    @pytest.mark.asyncio
    @pytest.mark.auth
    async def test_successful_login(self, mock_successful_login_response, sample_credentials, tmp_path):
    ...
    @pytest.mark.asyncio
    @pytest.mark.auth
    async def test_login_requires_mfa(self, mock_mfa_required_response, sample_credentials, tmp_path):
    ...
    async def test_login_missing_credentials(self, tmp_path):
        ...
        with pytest.raises(LoginFailedException, match="Email and password are required"):
            await mm.login("", "")
    ...
    @pytest.mark.asyncio
    @pytest.mark.auth
    async def test_login_with_rate_limiting(self, mock_rate_limit_response, sample_credentials, tmp_path):
...
class TestMFA:
    ...
    @pytest.mark.asyncio
    @pytest.mark.auth
    async def test_mfa_with_email_otp(self, mock_successful_login_response, sample_credentials, tmp_path):
    ...
    @pytest.mark.asyncio
    @pytest.mark.auth
    async def test_mfa_with_totp(self, mock_successful_login_response, sample_credentials, tmp_path):
...
class TestGraphQLFallback:
    ...
    @pytest.mark.asyncio
    @pytest.mark.auth
    async def test_login_404_triggers_graphql_fallback(self, sample_credentials, tmp_path):
        ...
        with patch('aiohttp.ClientSession') as mock_session_class:
            ...
            with patch.object(mm, '_login_user_graphql', new_callable=AsyncMock) as mock_graphql:
                await mm._login_user(sample_credentials["email"], sample_credentials["password"], None)
    ...
    @pytest.mark.asyncio
    @pytest.mark.auth
    async def test_mfa_404_triggers_graphql_fallback(self, sample_credentials, tmp_path):
        ...
        with patch('aiohttp.ClientSession') as mock_session_class:
            ...
            with patch.object(mm, '_mfa_graphql', new_callable=AsyncMock) as mock_mfa_graphql:
...
class TestHeaderGeneration:
    ...
    def test_device_uuid_is_valid_uuid(self):
        ...
        assert re.match(uuid_pattern, device_uuid), f"Invalid UUID format: {device_uuid}"
    ...
    def test_token_sets_authorization_header(self):
        ...
        assert "Authorization" not in mm._headers or mm._headers.get("Authorization") == ""
